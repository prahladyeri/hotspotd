...
def configure():
    ...
    if s is not None:
        ...
        # print 'and it is:'  + s
        for line in lines:
            if (not line.startswith(' ') and not line.startswith('mon.') and
                    'IEEE 802.11' in line):
                wlan =line.split(' ')[0]
    ...
    for line in lines:
        if (not line.startswith(' ') and
                not line.startswith(wlan) and
                not line.startswith('lo') and
                not line.startswith('mon.') and len(line) > 0):
            iface.append(line.split(' ')[0])
            # print 'f::' + line

    if not len(iface):
        print 'No network nic could be found on ' \
              'your deivce to interface with the LAN'
    elif len(iface) == 1:
        ppp = iface[0]
    ...
    dc = {'wlan': wlan, 'inet': ppp, 'ip': IP,
          'netmask': Netmask, 'SSID': SSID, 'password': password}
...
def check_dependencies():
    ...
    if not len(cli.check_sysfile('hostapd')):
        print 'hostapd executable not found. ' \
              'Make sure you have installed hostapd.'
        return False
    elif not len(cli.check_sysfile('dnsmasq')):
        print 'dnsmasq executable not found. ' \
...
def check_interfaces():
    ...
    if not bwlan:
        ...
        print ppp + ' interface was not found. ' \
...
def pre_start():
    ...
    try:
        ...
        # 29-12-2014: Rather than patching individual distros,
        # lets make it a default.
...
def start_router():
    ...
    if len(r.strip()) > 0:
        print r.strip()
    cli.execute_shell('iptables -t nat -A POSTROUTING -o ' + ppp +
                      ' -j MASQUERADE')
    cli.execute_shell('iptables -A FORWARD -i ' + ppp + ' -o ' + wlan +
                      ' -j ACCEPT -m state --state RELATED,ESTABLISHED')
    cli.execute_shell('iptables -A FORWARD -i ' + wlan + ' -o ' + ppp +
                      ' -j ACCEPT')

    # allow traffic to/from wlan
    cli.execute_shell('iptables -A OUTPUT --out-interface ' + wlan +
                      ' -j ACCEPT')
    cli.execute_shell('iptables -A INPUT --in-interface ' + wlan +
                      ' -j ACCEPT')

    # start dnsmasq
    s = 'dnsmasq --dhcp-authoritative --interface=' + wlan +\
        ' --dhcp-range=' + ipparts + '.20,' + ipparts + '.100,' +\
        Netmask + ',4h'
...
def stop_router():
    """Stop the router."""
    # b ring down the interface
    cli.execute_shell('ifconfig mon.' + wlan + ' down')

    # TODO: Find some workaround. killing hostapd brings down the wlan0
    ...
    # stop dnsmasq
    if cli.is_process_running('dnsmasq') > 0:
        cli.write_log('stopping dnsmasq')
        cli.execute_shell('killall dnsmasq')

    # disable forwarding in iptables.
    cli.write_log('disabling forward rules in iptables.')
    cli.execute_shell('iptables -P FORWARD DROP')

    # delete iptables rules that were added for wlan traffic.
    if wlan is not None:
        cli.execute_shell('iptables -D OUTPUT --out-interface ' + wlan +
                          ' -j ACCEPT')
        cli.execute_shell('iptables -D INPUT --in-interface ' + wlan +
                          ' -j ACCEPT')
    cli.execute_shell('iptables --table nat --delete-chain')
    cli.execute_shell('iptables --table nat -F')
    cli.execute_shell('iptables --table nat -X')
    # disable forwarding in sysctl.
    cli.write_log('disabling forward in sysctl.')
    r = cli.set_sysctl('net.ipv4.ip_forward', '0')
    print r.strip()
    # cli.execute_shell('ifconfig ' + wlan + ' down'  + IP + ' netmask '
    #  + Netmask)
...
def main(args):
    """Main function."""
    global wlan, ppp, IP, Netmask
    with open("VERSION", 'r', encoding='utf-8') as vesion_file:
        the_version = vesion_file.read().strip()
    print "****"
    ...
    if not os.path.exists('hotspotd.json'):
        ...
        newconfig = True
    if not len(cli.check_sysfile('hostapd')):
        print "hostapd is not installed on your system. This package will" \
              " not work without it.\nTo install hostapd, run 'sudo apt-get" \
              " install hostapd'\nor refer to" \
    ...
    if args.command == 'configure':
        ...
        if (cli.is_process_running('hostapd') and
                cli.is_process_running('dnsmasq')):
